...
# MC
c3 = montecarlo_expectation(cost,u0s,ps,prob,Tsit5(),EnsembleThreads();trajectories=100000,saveat=0.1)
@test abs(c1 - c3) < 0.1
...
# ps  = (Truncated(Normal(1.25, 0.25),0.5, 2.0),)
#
# @btime koopman_expectation2(cost,u0s,ps,prob,Tsit5();quadalg=CubatureJLh(), batch=1,
...
# ps  = [Truncated(Normal(1.25, 0.25),0.5, 2.0),]
#
# @btime sol = koopman_expectation2(cost,u0s,ps,prob,Tsit5();quadalg=CubatureJLh(), batch=1,
#                          iabstol=1e-3,ireltol=1e-3,
#                          maxiters=2000,saveat=0.1)
# @show sol.u
